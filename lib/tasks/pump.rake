  ...
  task(:pump => :environment) do
    ...
    @superadmin=User.find_by_login("boss")
		if Workspace.find_by_creator_id_and_state(@superadmin.id, "private").blank?
			@ws=Workspace.create(:creator_id => @superadmin.id, :description => "Private Workspace for Boss", :title=> "Private for Boss", :state => "private", :ws_items => @default_conf['sa_items'].to_a.join(','), :ws_item_categories => @default_conf['sa_item_categories'].to_a.join(','))
    ...
    if Workspace.find_by_creator_id_and_state(@user.id, "private").blank?
      @ws=Workspace.create(:creator_id => @user.id, :description => "Private Workspace for Quentin", :title=> "Private for Quentin", :state => "private", :ws_items => @default_conf['sa_items'].to_a.join(','), :ws_item_categories => @default_conf['sa_item_categories'].to_a.join(','))
