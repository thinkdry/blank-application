      ...
      def acts_as_authorizable
				...
				if ITEMS.include?(self.to_s.underscore)
					named_scope :matching_user_with_permission_in_containers, lambda { |user, permission, container_ids, container|
						...
						container_ids ||= container.classify.constantize.allowed_user_with_permission(user, self.to_s.underscore + '_' + permission, container).find(:all, :select => "#{container.pluralize}.id, #{container.pluralize}.title", :include => :user).map{ |e| e.id }
						container_ids = container_ids.map{|w_id| w_id.to_i} & container.classify.constantize.allowed_user_with_permission(user, self.to_s.underscore + '_' + permission, container).find(:all, :select => "#{container.pluralize}.id, #{container.pluralize}.title", :include => :user).map{ |e| e.id }
