...
class AuditObserver < ActiveRecord::Observer
  ...
  observe Audit
   ...
   def after_create(audit)   
      ...
      action_id = NotificationFilter.actions.find_by_name(audit.audit_action).id
                  
      subscribers = User.subscribers_of(model_id,action_id)      
      for subscriber in subscribers
        args = [subscriber.email,
                subscriber.class.to_s.downcase,
                'vincent@thinkdry.com',
                'Evolution du back office', 
                 audit.id,subscriber.id]
