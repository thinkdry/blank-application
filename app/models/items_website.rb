class ItemsWebsite < ActiveRecord::Base
  ...
  # Relation 1-N with the 'workspaces' table
        ...
        named_scope :allowed_user_with_permission_in_website, lambda { |user_id, permission_name, website_ids|
          ...
          if User.find(user_id).has_system_role('superadmin')
            ...
            { :select => 'DISTINCT items_websites.*',
              :joins => #"LEFT JOIN workspaces ON items.workspace_id IN (#{workspace_ids.split(',')}) "+
              "LEFT JOIN users_containers ON users_containers.containerable_id IN (#{website_ids.split(',')}) AND users_containers.containerable_type = 'Website' AND users_workspaces.user_id = #{user_id.to_i} "+
