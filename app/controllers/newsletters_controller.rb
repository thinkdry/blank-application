class NewslettersController < ApplicationController
    ...
    if GroupsNewsletter.new(:group_id => @group.id,:newsletter_id => @newsletter.id,:sent_on=>Time.now).save
      for member in @group.members
        if member.newsletter
         args = [member.email,member.class.to_s.downcase,get_sa_config['sa_contact_email'],@newsletter.title, @newsletter.description, @newsletter.body]
  ...
  def unsubscribe
    ...
    if @member.update_attributes(:newsletter => false)
      ...
      redirect_to get_sa_config['sa_application_url']
    end
  end
end
