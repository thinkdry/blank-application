...
* Install required gems (list will come)
...
== Crontab configuration (production environment)
	...
	*/1 * * * *	cd /path/to/my/application && rake xapian:update_index RAILS_ENV=development
	
	# Import latest Pubmed publications each 20 minute
	*/20 * * * *	cd /path/to/my/application && rake cron:pubmed:update_rss
...
Publish a patch:
* git pull
* git show origin/master (to know the revision of origin)
* git format-patch (as arguments, the range of revisions you want to publish => see man git-format-patch)
